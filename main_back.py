...
class VotingPoll(BaseHandler):
    def get(self,league,year,week):
        ...
        self.render('voting.html', curr_week = curr_week, league_info = league_info,
                                week_url = week, tally = tally, records = records, 
...
class ChooseWinner(BaseHandler):
    def get(self,league,year,week):
        ...
        self.render('management/winners.html', curr_week = curr_week, league_info = league_info,
    ...
    def get(self,league,year):
        ...
        if league_info:
            ...
            for team in teams:
                ...
                for vote in team.choices:
                    tgv += 1
                    if vote.choice in weeks_info[vote.week]:
                        corrects[team_name] += 1
                    if vote.week not in weeks_played[team.team]:
                        weeks_played[team_name].append(int(vote.week))
                ...
                if tgv > 0:
                    vote_percent[team_name] = round((corrects[team_name]/tgv)*100,2)
                total_votes[team_name]= tgv

        self.render('season_tally.html', totals = total_votes,corrects = sorted(corrects.items(), key= lambda x: x[1], reverse=True), 
                                reddit_names = reddit_names,games = games, weeks_parti = weeks_parti, 
                                vote_percent=vote_percent, w = weeks_played)  
                              ...
                              # (r'^/([a-z]+)/year/([0-9]+)/add/matches/',AddMatches),
                               ...
                               (r'^/([a-z]+)/year/([0-9]+)/week/([0-9]+)/winners/',ChooseWinner),
                                (r'^/([a-z]+)/year/([0-9]+)/manage/',Management),
                                 #(r'^/([a-z]+)/year/([0-9]+)/all/alt/',AllVotes),
